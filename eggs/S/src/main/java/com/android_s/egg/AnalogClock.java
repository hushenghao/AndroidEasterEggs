...
//        saveAttributeDataForStyleable(context, R.styleable.AnalogClock,
...
//                R.styleable.AnalogClock_dialTint);
...
//                a.getInt(R.styleable.AnalogClock_dialTintMode, -1),
//                null);
...
//                R.styleable.AnalogClock_hand_hourTint);
...
//                a.getInt(R.styleable.AnalogClock_hand_hourTintMode, -1),
//                null);
...
//                R.styleable.AnalogClock_hand_minuteTint);
...
//                a.getInt(R.styleable.AnalogClock_hand_minuteTintMode, -1),
//                null);
...
//                R.styleable.AnalogClock_hand_secondTint);
...
//                a.getInt(R.styleable.AnalogClock_hand_secondTintMode, -1),
//                null);
...
//        mTimeZone = toZoneId(a.getString(R.styleable.AnalogClock_timeZone));
        createClock();
...
//    }
    ...
    protected void onTimeChanged() {
        Calendar now = now();
        onTimeChanged(now, now.getTime().getTime());
//        onTimeChanged(now.atZone(mClock.getZone()).toLocalTime(), now.toEpochMilli());
    }

    private void onTimeChanged(Calendar localTime, long nowMillis) {
        float rawSeconds = localTime.get(Calendar.SECOND);
        mSeconds =
                mSecondsHandFps <= 0
                        ? rawSeconds
                        : Math.round(rawSeconds * mSecondsHandFps) / (float) mSecondsHandFps;
        mMinutes = localTime.get(Calendar.MINUTE) + mSeconds / 60.0f;
        mHour = localTime.get(Calendar.HOUR_OF_DAY) + mMinutes / 60.0f;
