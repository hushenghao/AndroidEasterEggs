...
class UniverseProgressNotifier(val context: Context, val universe: Universe) {
    ...
    private val isProgressNotifierSupported = Build.VERSION.SDK_INT >= Build.VERSION_CODES.CINNAMON_BUN
    ...
    init {
        if (isProgressNotifierSupported) {
            val spacecraftIcon = Icon.createWithResource(context, R.drawable.cinnamon_bun_ic_spacecraft_filled)
            progress = Notification.ProgressStyle().setProgressTrackerIcon(spacecraftIcon)
            planetIcons = listOf(
                (lerpRange(PLANET_RADIUS_RANGE, 0.75f)) to
                        Icon.createWithResource(context, R.drawable.cinnamon_bun_ic_planet_large),
                (lerpRange(PLANET_RADIUS_RANGE, 0.5f)) to
                        Icon.createWithResource(context, R.drawable.cinnamon_bun_ic_planet_medium),
                (lerpRange(PLANET_RADIUS_RANGE, 0.25f)) to
                        Icon.createWithResource(context, R.drawable.cinnamon_bun_ic_planet_small),
                (PLANET_RADIUS_RANGE.start to
                        Icon.createWithResource(context, R.drawable.cinnamon_bun_ic_planet_tiny)),
            )
        }
    ...
    private fun onSimulationStep() {
        if (universe.now - lastUpdate >= UPDATE_FREQUENCY_SEC) {
            ...
            builder
                ...
                .setSmallIcon(R.drawable.cinnamon_bun_ic_spacecraft_rotated)
