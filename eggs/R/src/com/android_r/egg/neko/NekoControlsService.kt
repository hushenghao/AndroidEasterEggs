...
val P_TOY_ICONS = intArrayOf(
        1, R.drawable.r_ic_toy_mouse,
        1, R.drawable.r_ic_toy_fish,
        1, R.drawable.r_ic_toy_ball,
        1, R.drawable.r_ic_toy_laser
)
...
public class NekoControlsService : ControlsProviderService(), PrefState.PrefsListener {
    ...
    private fun makeToyControl(icon: Icon?, thrown: Boolean): Control {
        return Control.StatefulBuilder(CONTROL_ID_TOY, getPendingIntent())
                ...
                .setTitle(colorize(getString(R.string.r_control_toy_title), COLOR_TOY_FG))
                .setStatusText(colorize(
                        if (thrown) getString(R.string.r_control_toy_status) else "",
                        COLOR_TOY_FG))
                .setControlTemplate(StatelessTemplate("toy"))
                .setStatus(Control.STATUS_OK)
                .setSubtitle(if (thrown) "" else getString(R.string.r_control_toy_subtitle))
    ...
    private fun makeWaterBowlControl(fillLevel: Float): Control {
        return Control.StatefulBuilder(CONTROL_ID_WATER, getPendingIntent())
                ...
                .setTitle(colorize(getString(R.string.r_control_water_title), COLOR_WATER_FG))
                .setCustomColor(ColorStateList.valueOf(COLOR_WATER_BG))
                .setCustomIcon(Icon.createWithResource(this,
                        if (fillLevel >= 100f) R.drawable.r_ic_water_filled else R.drawable.r_ic_water))
                ...
                .setControlTemplate(RangeTemplate("waterlevel", 0f, 200f, fillLevel, 10f,
                        "%.0f mL"))
                .setStatus(Control.STATUS_OK)
                .setSubtitle(if (fillLevel == 0f) getString(R.string.r_control_water_subtitle) else "")
                .build()
    }
    ...
    private fun makeFoodBowlControl(filled: Boolean): Control {
        return Control.StatefulBuilder(CONTROL_ID_FOOD, getPendingIntent())
                ...
                .setTitle(colorize(getString(R.string.r_control_food_title), COLOR_FOOD_FG))
                .setCustomIcon(Icon.createWithResource(this,
                        if (filled) R.drawable.r_ic_foodbowl_filled else R.drawable.r_ic_bowl))
                        // .setTint(COLOR_FOOD_FG)) // TODO(b/159559045): uncomment when fixed
                .setStatusText(
                        if (filled) colorize(
                                getString(R.string.r_control_food_status_full), 0xCCFFFFFF.toInt())
                        else colorize(
                                getString(R.string.r_control_food_status_empty), 0x80FFFFFF.toInt()))
                .setControlTemplate(ToggleTemplate("foodbowl", ControlButton(filled, "Refill")))
                .setStatus(Control.STATUS_OK)
                .setSubtitle(if (filled) "" else getString(R.string.r_control_food_subtitle))
                .build()
    }
