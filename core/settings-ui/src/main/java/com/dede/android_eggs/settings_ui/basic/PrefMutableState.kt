...
@Composable
fun rememberPrefIntState(key: PrefKey<Int>, default: Int = key.default): MutableIntState {
    return remember(key, default) { mutablePrefIntState(key, default) }
}

@Composable
fun rememberPrefBoolState(key: PrefKey<Boolean>): MutableState<Boolean> {
    return remember(key) { mutablePrefBoolState(key) }
}

fun mutablePrefColorState(key: PrefKey<Int>): MutableState<Color> {
    return PrefMutableColorState(globalContext, key.name, Color(key.default))
}

/**
 * [default] overrides the value declared on [key] for settings whose default
 * depends on device capabilities, see `ColorSourcePrefUtil.DEFAULT_SOURCE`.
 */
fun mutablePrefIntState(key: PrefKey<Int>, default: Int = key.default): MutableIntState {
    return PrefMutableIntState(globalContext, key.name, default)
}

internal fun mutablePrefBoolState(key: PrefKey<Boolean>): MutableState<Boolean> {
    return PrefMutableBooleanState(globalContext, key.name, key.default)
}
