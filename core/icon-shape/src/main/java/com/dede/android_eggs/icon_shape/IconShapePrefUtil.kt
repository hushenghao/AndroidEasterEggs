...
object IconShapePrefUtil {
    ...
    fun getIconShape(): Shape {
        val index: Int = if (LocalInspectionMode.current) {
            ...
            IconShapePreference.selectedIndex.intValue
        }
        return shapeSpecs.getOrElse(index) { circleShapeSpec }.toShape()
    }
    ...
    /**
     * The shapes the user can pick from. The selection is stored as an index into
     * this list (`pref_key_override_icon_shape`), so new shapes are appended at the
     * end: inserting one in the middle renumbers everything after it and silently
     * changes the shape of every existing install.
     */
