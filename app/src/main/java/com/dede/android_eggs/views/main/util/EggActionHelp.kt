...
object EggActionHelp {
    ...
    private fun findTaskWithTrim(context: Context, clazz: Class<out Activity>): AppTask? {
        val mainComponent = Utils.getLaunchIntent(context)?.component ?: return null

        fun AppTask.isThisTask(target: String): Boolean {
            return taskInfo?.baseIntent?.component?.className == target
        }
        ...
        for (task in activityManager.appTasks) {
            if (task.isThisTask(mainComponent.className)) {
                // exclude main task
                continue
            }
            if (task.isThisTask(clazz.name)) {
